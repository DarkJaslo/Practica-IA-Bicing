#Comparacion de operadores

#Lectura de datos
setwd("C:/Users/Sandra/Documents/Uni/IA/Lab")
if (!require("data.table")) install.packages("data.table")
library(data.table)
res2 <- read.table("exp2.txt", header = TRUE, sep = "\t")

sol1 <- subset(res2, sol_ini=="Vacia")
sol2 <- subset(res2, sol_ini=="Normal")
sol3 <- subset(res2, sol_ini=="Greedy")

#Comparando beneficios
boxplot(sol1$calidad, sol2$calidad, sol3$calidad, names=c("Vacia", "Normal", "Greedy"), ylab="beneficio")
dif12 <- sum(sol1$benef > sol2$benef)

#Probabilidad de que pase esto bajo H0
dbinom(x=dif12,size=length(sol1$benef),prob=0.5)

#Comparando tiempos
boxplot(sol1$tiempo, sol2$tiempo, sol3$tiempo, names=c("Vacia", "Normal", "Greedy"), ylab="ms")
t.test(sol1$tiempo, sol2$tiempo, paired=TRUE)